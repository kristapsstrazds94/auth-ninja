  ...
  if (result.status !== 0) {
    throw new Error(
      "Could not start PostgreSQL via Docker. Either:\n" +
        ...
        "  • Run: pnpm --filter @auth-ninja/demo-e2e postgres:down && auth-ninja demo",
    );
  }
...
/** Resolve a working Postgres URL, falling back to Docker Compose when needed. */
export async function ensureDemoDatabase(repoRoot: string, configured?: string): Promise<string> {
